...
name: help
description: >-
  Capabilities overview for the RektFree plugin. Use when the user asks "what can
  you do", "what can this plugin do", "help", "capabilities", "what tools do you
  have", "where do I start", or otherwise wants a map of the plugin's commands,
  tools, and markets before diving into a specific analysis. Renders a concise,
  categorized overview — no MCP tool calls.
---

# RektFree — Capabilities Overview

The user wants to know what this plugin can do. Present a **scannable,
categorized map** of the commands — not a JSON dump, and do **not** call any MCP
tool. This is the same overview as the `/help` command.

Group the commands into these buckets, one short line per command (what it's for):

**Flagship reads** — synthesis commands that combine many tools into one brief
