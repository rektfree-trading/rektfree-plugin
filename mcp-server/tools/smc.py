"""
``analyze_smc`` — Smart Money Concepts analysis tool (crypto, keyless).

Fetches Binance OHLCV and runs the vendored SMC engine, returning structured
JSON that mirrors the backend's /market/smc payload shape.
"""
...
def _serialize(result: smart_money.SMCResult, times: list[float]) -> dict:
    ...
    n = len(times)
    return {
        "trend_bias": bias_str(result.trend_bias),
...
def register(mcp) -> None:
    ...
    async def analyze_smc(
        ...
        Fetches OHLCV candles from Binance (public, no API key needed) and runs
        the full SMC engine: market structure (BOS/CHoCH), order blocks, fair
        value gaps, equal highs/lows, liquidity sweeps, breaker blocks, and
        premium/discount range. Returns structured JSON for the model to
        interpret.
        ...
        Returns:
            A dict with ``symbol``, ``timeframe``, ``candle_count``,
            ``last_price``, and the full SMC breakdown (trend_bias, structures,
            order_blocks, fair_value_gaps, equal_levels, swing_labels,
